...
int main() {
    ...
    if (ShMemInit(shmem, sizeof(int), pathname) == -1) {
        printf("[boys] unable to create shared memory!\n");
        ShMemCheck(shmem);
        exit(-1);
    } else if (shmem->new == 1) {
        *(int*)(shmem->buf) = vacantMax;
        printf("[boys] shared memory chunk created!\n vacant places: %d\n", vacantMax);
    }
    ...
    if (semid == -1 && errno == EEXIST) {           // can't open existing semaphore
        ...
        if (semid == -1) {
            printf("[boys] can't get semaphore!\n");
            exit(-1);
        }
    ...
    printf("[boys] new BOI has arrived\n[boys] waiting for girls to leave the shower...\n");
    C_girls;
    ...
    V_boys;
    V_mutex;
    printf("[boys] entered the shower...\n[boys] washing...");
